...
static const int servoPin1 = 25;  // Pino de controle do servo BOM  (JOYSTICK DIREITA - CIMA E BAIXO) 
static const int servoPin2 = 26;  // Pino de controle do servo  BOM  (JOYSTICK DIREITA - DIREITA E ESQUERDA)
static const int servoPin4 = 27;  // Pino de controle do servo BOM   (JOYSTICK ESQUERDA - CIMA E BAIXO) motor da diteita (vista frontal)
static const int servoPin3 = 32;  // Pino de controle do servo  BOM    (JOYSTICK ESQUERDA - DIREITA E ESQUERDA) motor da esquerda  (vista frontal)
...
//BUZZER
  ...
  if (ctl->buttons() == 0x0040) {
    digitalWrite(13,HIGH);  // Emite um tom de 1000Hz
    // code for when L2 button is pushed
  }
  if (ctl->buttons() != 0x0040) {
    digitalWrite(13,LOW);      // Para o som
...
//== LEFT JOYSTICK - X AXIS ==// MOvimento responsável por mover a garra para frente e para trás
  if (ctl->axisX()) {
...
//== LEFT JOYSTICK - Y AXIS ==// movimento de descer e subir a garra
  if (ctl->axisY() ) {
    ...
    // Mapeia o valor do potenciômetro para o ângulo do servo (0 a 180)
    int angle = map(eixoly, -511, 512, 180, 0); 
